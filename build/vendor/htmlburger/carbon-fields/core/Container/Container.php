...
namespace FuerteWpDep\Carbon_Fields\Container;

use FuerteWpDep\Carbon_Fields\Carbon_Fields;
use FuerteWpDep\Carbon_Fields\Helper\Helper;
use FuerteWpDep\Carbon_Fields\Field\Field;
use FuerteWpDep\Carbon_Fields\Field\Group_Field;
use FuerteWpDep\Carbon_Fields\Container\Fulfillable\Fulfillable_Collection;
use FuerteWpDep\Carbon_Fields\Datastore\Datastore_Interface;
use FuerteWpDep\Carbon_Fields\Datastore\Datastore_Holder_Interface;
...
abstract class Container implements Datastore_Holder_Interface
{
     ...
     * @var boolean
     */
    protected $has_default_datastore = \true;
    /**
    ...
    public static function factory($raw_type, $id, $name = '')
    {
        ...
        if (Carbon_Fields::has($type, 'containers')) {
            $container = Carbon_Fields::resolve_with_arguments($type, array('id' => $id, 'name' => $name, 'type' => $type), 'containers');
        } else {
    ...
    public function __construct($id, $title, $type, $condition_collection, $condition_translator)
    {
        ...
        $this->condition_collection->set_condition_type_list(\array_merge($this->get_condition_types(\true), $this->get_condition_types(\false)), \true);
     ...
     * @return bool
     */
    protected final function _is_valid_save()
    {
     ...
     * @return bool
     */
    protected abstract function is_valid_save();
    /**
     ...
     * @return array
     */
    protected abstract function get_environment_for_request();
    /**
     * Check container attachment rules against current page request (in admin)
     *
     * @return bool
     */
    protected abstract function is_valid_attach_for_request();
    /**
    ...
    protected function static_conditions_pass()
    {
        ...
        $static_condition_collection = $this->condition_collection->evaluate($this->get_condition_types(\false), \true);
     ...
     * @return array
     */
    protected abstract function get_environment_for_object($object_id);
    /**
     ...
     * @return bool
     */
    public abstract function is_valid_attach_for_object($object_id);
    /**
        ...
        while (!empty($hierarchy_left)) {
            ...
            foreach ($field_group as $f) {
                ...
                if (empty($hierarchy_left)) {
                    ...
                    if (!$f instanceof \FuerteWpDep\Carbon_Fields\Field\Complex_Field) {
                        return null;
                    }
    ...
    public function set_datastore(Datastore_Interface $datastore, $set_as_default = \false)
    {
        if ($set_as_default && !$this->has_default_datastore()) {
            return $this;
            // datastore has been overriden with a custom one - abort changing to a default one
        }
        $this->datastore = $datastore;
        $this->has_default_datastore = $set_as_default;
        foreach ($this->fields as $field) {
            $field->set_datastore($this->get_datastore(), \true);
        }
        return $this;
    }
    /**
