...
class Post_Meta_Container extends Container
{
    ...
    protected function get_environment_for_request()
    {
        ...
        $post = $post ? $post : null;
        $environment = array('post_id' => $post ? $post->ID : 0, 'post_type' => $post ? $post->post_type : $post_type, 'post' => $post);
        return $environment;
    }
    /**
    ...
    public function attach()
    {
        ...
        foreach ($this->post_types as $post_type) {
            add_meta_box($this->get_id(), $this->title, array($this, 'render'), $post_type, $this->settings['meta_box_context'], $this->settings['meta_box_priority']);
    ...
    {
        include \FuerteWpDep\Carbon_Fields\DIR . '/templates/Container/post_meta.php';
    }
    /**
    ...
    public function get_post_type_visibility()
    {
        ...
        $evaluated_collection = $this->condition_collection->evaluate(array('post_type'), \true, array(), \true);
        $shown_on = array();
        foreach ($all_post_types as $post_type) {
            $environment = array('post_type' => $post_type);
    ...
    {
        $this->where('post_term', '=', array('value' => $category_slug, 'field' => 'slug', 'taxonomy' => 'category'));
        return $this;
    }
    /**
    ...
    {
        $this->where('post_term', '=', array('value' => $term_slug, 'field' => 'slug', 'taxonomy' => $taxonomy_slug));
        return $this;
    }
    /**
