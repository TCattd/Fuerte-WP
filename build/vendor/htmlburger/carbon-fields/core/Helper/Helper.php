...
class Helper
{
    ...
    {
        \FuerteWpDep\Carbon_Fields\Carbon_Fields::verify_fields_registered();
        $repository = \FuerteWpDep\Carbon_Fields\Carbon_Fields::resolve('container_repository');
        if ($container_id) {
    ...
    {
        return static::with_field_clone($object_id, $container_type, $container_id, $field_name, function ($field) {
            if (!$field) {
                return '';
            }
    ...
    {
        static::with_field_clone($object_id, $container_type, $container_id, $field_name, function ($field) use($container_type, $container_id, $field_name, $value) {
            if (!$field) {
    ...
    public static function get_relation_type_from_array($array, $allowed_relations = array('AND', 'OR'), $relation_key = 'relation')
    {
        ...
        if (!\in_array($relation, $allowed_relations)) {
            Incorrect_Syntax_Exception::raise('Invalid relation type ' . $relation . '. ' . 'The rule should be one of the following: "' . \implode('", "', $allowed_relations) . '"');
        }
        return $relation;
    }
    /**
    ...
    public static function get_attachment_id($url)
    {
        ...
        if (\strpos($url, $dir['baseurl'] . '/') !== \false) {
            $query_args = array('post_type' => 'attachment', 'post_status' => 'inherit', 'fields' => 'ids', 'meta_query' => array(array('value' => $filename, 'compare' => 'LIKE', 'key' => '_wp_attachment_metadata')));
    ...
    {
        $attachment_metadata = array('id' => 0, 'thumb_url' => '', 'file_type' => '', 'file_name' => '');
        ...
        $attachment_metadata['file_type'] = \preg_replace('~\\/.+$~', '', $attachment_metadata['filetype']['type']);
        // image, video, etc..
    ...
    public static function input()
    {
        ...
        if (\FuerteWpDep\Carbon_Fields\COMPACT_INPUT) {
    ...
    {
        if (isset($input[\FuerteWpDep\Carbon_Fields\COMPACT_INPUT_KEY])) {
            $inputs = $input[\FuerteWpDep\Carbon_Fields\COMPACT_INPUT_KEY];
    ...
    public static function get_valid_options($input, $options)
    {
        ...
        foreach ($input as $raw_value) {
            $index = \array_search(\strval($raw_value), $search_options, \true);
            if ($index === \false) {
                continue;
            }
