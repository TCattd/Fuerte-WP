...
# 1.12.0 / 2026-09-11
- **New Feature**: Filesystem kill switches for server operations. A `.fuertewp-disable` marker file in the WordPress root or one level above it (the docroot parent, where a moved `wp-config.php` would live) prevents the entire plugin from booting; `.fuertewp-disable-mfa` keeps only the bundled Two-Factor off. Presence only: an empty file counts and content is never read, so `touch .fuertewp-disable` is a complete emergency stop that survives plugin updates and DB changes. Both directories are checked and cached per request. (`Fuerte_Wp_Dot_Files`, tests/unit/DotFilesTest.php)
- **Note**: Parent-directory markers are site-wide. On shared hosting where several sites share one parent folder, a single marker disables Fuerte-WP for all of them.
- **Bug Fix**: The seven email-notification toggles on the E-mails tab (fatal error, automatic updates, comment awaiting moderation, comment published, password reset, personal-data export request, new user) were no-ops: they registered custom filter names (`disable_*_emails`) that exist nowhere in WordPress core, so the emails kept sending regardless of the checkboxes. Each toggle now maps to its real core filter (for example `notify_moderator`, `auto_plugin_update_send_email`, `wp_new_user_notification_email_admin`), suppressed by emptying the recipient, which `wp_mail()` rejects through its own PHPMailer guard. All mappings verified against WordPress core 7.0 source.
- **New Feature**: The three network email toggles are wired for the first time: new site created (`send_new_site_email`), new site activated (`wpmu_welcome_notification`), and new user site registered, which short-circuits core's `registrationnotification` gate because core ships no disable filter for `newuser_notify_siteadmin()`. Core ties the self-service signup notice (`newblog_notify_siteadmin()`) to the same gate, so unchecking "new user site registered" also silences that email.
- **New Feature**: Forward-adaptation for WordPress 7.2 (core.trac changeset 63582, ticket #63927): a new "Application password created" toggle controls the notification users get when an application password is added to their account. Default ON (critical security notice). The wiring stays completely inert until the running WordPress registers the feature, detected via core's own `wp_create_application_password` action registration rather than a version guess.
- **Bug Fix**: The fatal-error (recovery mode) email redirect replaced core's entire email array with just the recipient, dropping subject, message and headers (empty-subject email plus PHP warnings). It now merges the recipient into the array core provides.
- **Bug Fix**: A `wp-config-fuerte.php` that omitted the `emails` section while enabling the custom sender silently lost the sender rewrite and the recovery-email redirect: the email-hook gate only passed when an `emails` key existed, and file configs are not merged with defaults. The gate now also honors `general.sender_email_enable` and `general.recovery_email`.
- **Changed**: Email suppression is type-faithful per filter shape: boolean send filters receive `false`, content-array filters keep their payload with an emptied recipient, recipient-string filters receive an empty string.
- **Docs**: Escape-hatch documentation in `config-sample/wp-config-fuerte.php` and the knowledge base now covers the kill-switch layer, including the shared-hosting caveat. README and FAQ gained an emergency-stop section.
- Added 21 tests across four suites: real-filesystem kill-switch cases (both lookup directories, presence-only semantics, cache invalidation, 2FA gate integration), per-toggle core-hook mapping checks, the email-hook gate regression, and the suppression callback shapes. Suite: 190 passing.
- **Changed**: Bundled HyperFields library updated from 1.5.5 to 1.6.1; `composer.lock` synced and the shipped vendor stays dev-free. Upstream highlights: 1.6.1 adds `Field::toJsonSchema()` machine-readable field schemas, a single-field options writer (`OptionsPage::setFieldValue()`, same sanitize/validate pipeline as a form save, idempotent on unchanged values) and a WordPress 6.9+ Abilities API module (`hyperfields/list-option-pages`, `hyperfields/get-option`, `hyperfields/update-option`; permissions resolve per page, unknown pages fail closed); 1.6.0 fixes numeric-string select keys saving the wrong value; 1.5.5 ships security hardening (Data Tools drops CDN-loaded JS for bundled assets, HMAC log filename rotation, term-meta nonce and capability fixes, import sanitization seam). Dev-only packages refreshed; they are removed from the shipped vendor.
