...
class WPFuerte
{
	...
	protected function main()
	{
		 ...
		 */
		if ( isset( $this->wpfuerte['general']['autoupdate_core'] ) && true === $this->wpfuerte['general']['autoupdate_core'] ) {
		...
		if ( isset( $this->wpfuerte['general']['autoupdate_plugins'] ) && true === $this->wpfuerte['general']['autoupdate_plugins'] ) {
			add_filter( 'auto_update_plugin', '__return_true', 9999 );
		}

		if ( isset( $this->wpfuerte['general']['autoupdate_themes'] ) && true === $this->wpfuerte['general']['autoupdate_themes'] ) {
			add_filter( 'auto_update_theme', '__return_true', 9999 );
		}

		if ( isset( $this->wpfuerte['general']['autoupdate_translations'] ) && true === $this->wpfuerte['general']['autoupdate_translations'] ) {
		 ...
		 * Disable XML-RPC API
		 */
		if ( isset( $this->wpfuerte['general']['disable_xmlrpc'] ) && true === $this->wpfuerte['general']['disable_xmlrpc'] ) {
		 ...
		 */
		if ( isset( $this->wpfuerte['emails']['comment_awaiting_moderation'] ) && false === $this->wpfuerte['emails']['comment_awaiting_moderation'] ) {
			add_filter( 'notify_moderator', '__return_false', 9999 );
		}

		if ( isset( $this->wpfuerte['emails']['comment_has_been_published'] ) && false === $this->wpfuerte['emails']['comment_has_been_published'] ) {
			add_filter( 'notify_post_author', '__return_false', 9999 );
		}

		if ( isset( $this->wpfuerte['emails']['user_reset_their_password'] ) && false === $this->wpfuerte['emails']['user_reset_their_password'] ) {
			remove_action( 'after_password_reset', 'wp_password_change_notification', 9999 );
		}

		if ( isset( $this->wpfuerte['emails']['user_confirm_personal_data_export_request'] ) && false === $this->wpfuerte['emails']['user_confirm_personal_data_export_request'] ) {
			remove_action( 'user_request_action_confirmed', '_wp_privacy_send_request_confirmation_notification', 9999 );
		}

		if ( isset( $this->wpfuerte['emails']['automatic_updates'] ) && false === $this->wpfuerte['emails']['automatic_updates'] ) {
		...
		if ( isset( $this->wpfuerte['emails']['new_user_created'] ) && false === $this->wpfuerte['emails']['new_user_created'] ) {
		...
		if ( isset( $this->wpfuerte['emails']['network_new_site_created'] ) && false === $this->wpfuerte['emails']['network_new_site_created'] ) {
			add_filter( 'send_new_site_email', '__return_false', 9999 );
		}

		if ( isset( $this->wpfuerte['emails']['network_new_user_site_registered'] ) && false === $this->wpfuerte['emails']['network_new_user_site_registered'] ) {
			add_filter( 'wpmu_signup_blog_notification', '__return_false', 9999 );
		}

		if ( isset( $this->wpfuerte['emails']['network_new_site_activated'] ) && false === $this->wpfuerte['emails']['network_new_site_activated'] ) {
			remove_action( 'wp_initialize_site', 'newblog_notify_siteadmin', 9999 );
		}

		if ( isset( $this->wpfuerte['emails']['fatal_error'] ) && false === $this->wpfuerte['emails']['fatal_error'] ) {
		...
		if ( ! in_array( strtolower( $current_user->user_email ), $this->wpfuerte['super_users'] ) || true === WPFUERTE_FORCE ) {
			...
			if ( is_admin() ) {
				...
				if ( isset( $this->wpfuerte['general']['disable_app_passwords'] ) && true === $this->wpfuerte['general']['disable_app_passwords'] ) {
				...
				if ( isset( $this->wpfuerte['general']['disable_admin_create_edit'] ) && true === $this->wpfuerte['general']['disable_admin_create_edit'] ) {
					add_filter( 'editable_roles', array(__CLASS__, 'create_edit_role_check'), 9999 );
				}

				// Disallowed wp-admin scripts
				if ( isset( $this->wpfuerte['restricted_scripts'] ) && in_array( $this->pagenow, $this->wpfuerte['restricted_scripts'] ) && ! wp_doing_ajax() ) {
					$this->access_denied();
				}

				// Disallowed wp-admin pages
				if ( isset( $this->wpfuerte['restricted_pages'] ) && isset( $_REQUEST['page'] ) && in_array( $_REQUEST['page'], $this->wpfuerte['restricted_pages'] ) && !wp_doing_ajax() ) {
					$this->access_denied();
				}
				...
				if ( isset( $this->wpfuerte['general']['restrict_acf'] ) && true === $this->wpfuerte['general']['restrict_acf'] ) {
					// No ACF editor menu
				...
				if ( isset( $this->wpfuerte['general']['restrict_permalinks'] ) && true === $this->wpfuerte['general']['restrict_permalinks'] ) {
					// No Permalinks config access
					if ( in_array( $this->pagenow, ['options-permalink.php'] ) ) {
						$this->access_denied();
					}
				}
			} // is_admin()
			...
			if ( ! is_admin() ) {
				...
				if ( isset( $this->wpfuerte['general']['disable_admin_bar_roles'] ) && ! empty( $this->wpfuerte['general']['disable_admin_bar_roles'] ) ) {
					$roles_array = [];
	...
	private function self_protect()
	{
		...
		add_filter( 'plugin_action_links', function ( $actions, $plugin_file ) {
			if ( plugin_basename( WPFUERTE_PLUGIN_BASE ) === $plugin_file ) {
		...
		if ( isset( $_REQUEST['action'] )
			...
			&& stripos( $_REQUEST['plugin'], 'wp-fuerte' ) !== false
		) {
			$this->access_denied();
		}
	}
	...
	{
		if ( ! isset( $this->wpfuerte['general']['access_denied_message'] ) || empty( $this->wpfuerte['general']['access_denied_message'] ) ) {
			$this->wpfuerte['general']['access_denied_message'] = 'Access denied.';
		}
	...
	static function sender_email_address()
	{
		...
		if ( ! isset ( $wpfuerte['general']['sender_email'] ) || empty( $wpfuerte['general']['sender_email'] ) ) {
	...
	static function recovery_email_address()
	{
		...
		if ( ! isset ( $wpfuerte['general']['recovery_email'] ) || empty( $wpfuerte['general']['recovery_email'] ) ) {
	...
	?>
		<script type="text/javascript">
			...
			if ( isset( $wpfuerte['general']['force_strong_passwords'] ) && true === $wpfuerte['general']['force_strong_passwords'] ) :
			?>
	...
	?>
		...
		// Needed outside wp-admin, because reset password screen
		if ( isset( $wpfuerte['general']['disable_weak_passwords'] ) && true === $wpfuerte['general']['disable_weak_passwords'] ) :
		?>
		...
		if ( isset( $wpfuerte['general']['restrict_acf'] ) && true === $wpfuerte['general']['restrict_acf'] ) :
		?>
	...
	static function custom_login_logo()
	{
		...
		if ( isset( $wpfuerte['general']['use_site_logo_login'] ) && true === $wpfuerte['general']['use_site_logo_login'] ) {
			if ( ! has_custom_logo() ) {
				return;
			}
