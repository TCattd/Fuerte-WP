...
class Two_Factor_Core
{
     ...
     * @type string
     */
    public const PROVIDER_USER_META_KEY = '_two_factor_provider';
     ...
     * @type string
     */
    public const ENABLED_PROVIDERS_USER_META_KEY = '_two_factor_enabled_providers';
     ...
     * @type string
     */
    public const USER_META_NONCE_KEY = '_two_factor_nonce';
     ...
     * @type string
     */
    public const USER_RATE_LIMIT_KEY = '_two_factor_last_login_failure';
     ...
     * @var string
     */
    public const USER_FAILED_LOGIN_ATTEMPTS_KEY = '_two_factor_failed_login_attempts';
     ...
     * @var string
     */
    public const USER_PASSWORD_WAS_RESET_KEY = '_two_factor_password_was_reset';
     ...
     * @var string
     */
    public const USER_SETTINGS_ACTION_QUERY_VAR = 'two_factor_action';
     ...
     * @var string
     */
    public const USER_SETTINGS_ACTION_NONCE_QUERY_ARG = '_two_factor_action_nonce';
     ...
     * @var string
     */
    public const REST_NAMESPACE = 'two-factor/1.0';
    ...
    public static function add_hooks($compat)
    {
        ...
        add_action('init', [__CLASS__, 'get_providers']); // @phpstan-ignore return.void

        add_filter('wp_login_errors', [__CLASS__, 'maybe_show_reset_password_notice']);
        add_action('after_password_reset', [__CLASS__, 'clear_password_reset_notice']);
        add_action('login_form_validate_2fa', [__CLASS__, 'login_form_validate_2fa']);
        add_action('login_form_revalidate_2fa', [__CLASS__, 'login_form_revalidate_2fa']);
        ...
        add_filter('authenticate', [__CLASS__, 'filter_authenticate'], 31);

        // 2. Render two-factor UI after WP core has validated username/password during `wp_signon()`.
        add_action('wp_login', [__CLASS__, 'wp_login'], PHP_INT_MAX, 2);
        add_action('user_profile_update_errors', [__CLASS__, 'action_user_profile_update_errors']);
        ...
        add_filter('attach_session_information', [__CLASS__, 'filter_session_information'], 10, 2);

        add_action('login_enqueue_scripts', [__CLASS__, 'login_enqueue_scripts'], 5);
        ...
        add_filter('plugin_action_links_' . plugin_basename(TWO_FACTOR_DIR . 'two-factor.php'), [__CLASS__, 'add_settings_action_link']);
            ...
            if (method_exists($provider_class, 'uninstall_user_meta_keys')) {
                try {
                    $user_meta_keys = array_merge(
                        $user_meta_keys,
                        call_user_func([$provider_class, 'uninstall_user_meta_keys'])
                    );
            ...
            if (method_exists($provider_class, 'uninstall_options')) {
                try {
                    $option_keys = array_merge(
                        $option_keys,
                        call_user_func([$provider_class, 'uninstall_options'])
                    );
                } catch (Exception $e) { // phpcs:ignore Generic.CodeAnalysis.EmptyStatement.DetectedCatch -- Intentionally empty, provider may not implement this method.
                }
            }
    ...
    public static function get_providers()
    {
        ...
        foreach ($providers as $provider_key => $provider_class) {
            try {
                $providers[$provider_key] = call_user_func([$provider_class, 'get_instance']);
            } catch (Exception $e) {
                unset($providers[$provider_key]);
            }
        }
    ...
    public static function is_user_api_login_enabled($user_id)
    {
         ...
         * @since 0.4.0
         *
         * @param bool $enabled Whether the user can login via API requests.
         * @param int $user_id User ID.
         */
    ...
    public static function login_html($user, $login_nonce, $redirect_to, $error_msg = '', $provider = null, $action = 'validate_2fa')
    {
        ...
        $available_providers = self::get_available_providers_for_user($user);
        $backup_providers = array_diff_key($available_providers, [$provider_key => null]);
		...
		<form name="validate_2fa_form" id="loginform" action="<?php echo esc_url(self::login_url(['action' => $action], 'login_post')); ?>" method="post" autocomplete="off">
    ...
    public static function disable_provider_for_user($user_id, $provider_to_delete)
    {
        ...
        $enabled_providers = array_diff($enabled_providers, [$provider_to_delete]);
     ...
     * @return bool
     */
    public static function update_current_user_session($data = [])
    {
        $user_id = get_current_user_id();
